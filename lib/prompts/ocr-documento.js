...
ANTI-ALUCINACAO (REGRA CRITICA - leia 2 vezes):
- LEIA O CPF DIGITO POR DIGITO. Faca uma primeira leitura, depois LEIA DE NOVO comparando cada digito.
- Se houver QUALQUER duvida em algum digito (por exemplo entre 1 e 3, 6 e 8, 0 e 8), retorne "" (string vazia) ao inves de chutar.
- NUNCA INVENTE OU SUBSTITUA DIGITOS. Se a imagem esta borrada num digito especifico, retorne "".
- Apos extrair o CPF, RELEIA o documento e confirme que cada digito bate. Se nao bater, retorne "".
- Em "observacoes_leitura", confirme: "CPF foi lido 2x e validado digito-a-digito" OU "CPF nao foi extraido por falta de certeza absoluta".
- NAO TROQUE digitos parecidos: 1↔7, 3↔8, 5↔6, 0↔8, 0↔6.
- Lembre-se: digitar errado é MUITO PIOR que nao digitar.
...
export const SYSTEM_OCR_DOCUMENTO = "Voce eh um motor de OCR. Retorne APENAS JSON conforme o esquema solicitado. Sem markdown, sem texto antes ou depois.";
export function buildUserOCR({ tipoEsperado }) {
  return getOcrPrompt(tipoEsperado || 'desconhecido');
}
