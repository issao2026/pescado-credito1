...
(function(){
  ...
  window.verComposicaoKPI = function(tipo){
    ...
    if (tipo === 'total'){
      ...
      cL.slice(0,10).forEach(r=>{ h += '<tr onclick="verFichaLagostao(0,\''+r.cnpj+'\')" style="border-bottom:1px solid #EEF2F1;cursor:pointer;transition:.12s" onmouseover="this.style.background=\'#F5F7F6\'" onmouseout="this.style.background=\'\'"><td style="padding:8px 12px;color:#0A3332;font-weight:600">'+r.cliente+'</td><td style="padding:8px 12px;text-align:right">'+(r.score||'-')+'</td><td style="padding:8px 12px;text-align:right;font-weight:700">'+R$dr(r.limite)+'</td></tr>'; });
      h += '</tbody></table>';
    }
  ...
  // v4.40b: modal com lista dos clientes de um vendedor especifico
  window.verClientesDoVendedor = function(vendedor){
    var L = (window.LAGOSTAO_DATA || []);
    var clientes = L.filter(function(r){ return r.vendedor === vendedor; })
                    .sort(function(a,b){ return (b.limite||0) - (a.limite||0); });
    var totLimite = clientes.reduce(function(a,b){ return a + (b.limite||0); }, 0);
    var scoreMedios = clientes.filter(function(r){ return r.score > 0; }).map(function(r){ return r.score; });
    var scMed = scoreMedios.length ? Math.round(scoreMedios.reduce(function(a,b){return a+b;},0)/scoreMedios.length) : '—';
    var h = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px">';
    h += '<div style="background:#F5F7F6;padding:12px 14px;border-radius:8px"><div style="font-size:10px;color:#9896A8;text-transform:uppercase;font-weight:700">Clientes</div><div style="font-family:Poppins,sans-serif;font-size:22px;font-weight:800;color:#012A2A;margin-top:4px">' + clientes.length + '</div></div>';
    h += '<div style="background:#F5F7F6;padding:12px 14px;border-radius:8px"><div style="font-size:10px;color:#9896A8;text-transform:uppercase;font-weight:700">Total limite</div><div style="font-family:Poppins,sans-serif;font-size:22px;font-weight:800;color:#059669;margin-top:4px">R$ ' + (totLimite/1000).toFixed(0) + 'k</div></div>';
    h += '<div style="background:#F5F7F6;padding:12px 14px;border-radius:8px"><div style="font-size:10px;color:#9896A8;text-transform:uppercase;font-weight:700">Score medio</div><div style="font-family:Poppins,sans-serif;font-size:22px;font-weight:800;color:#012A2A;margin-top:4px">' + scMed + '</div></div>';
    h += '</div>';
    h += '<div style="max-height:420px;overflow-y:auto;border:1px solid #EEF2F1;border-radius:8px"><table style="width:100%;border-collapse:collapse;font-size:12.5px"><thead style="background:#F5F7F6;position:sticky;top:0"><tr><th style="padding:10px 14px;text-align:left;font-size:11px;color:#5F7573;text-transform:uppercase">Cliente</th><th style="padding:10px 14px;text-align:left;font-size:11px;color:#5F7573;text-transform:uppercase">CNPJ</th><th style="padding:10px 14px;text-align:right;font-size:11px;color:#5F7573;text-transform:uppercase">Score</th><th style="padding:10px 14px;text-align:right;font-size:11px;color:#5F7573;text-transform:uppercase">Limite</th><th style="padding:10px 14px;text-align:right;font-size:11px;color:#5F7573;text-transform:uppercase">Prazo</th></tr></thead><tbody>';
    clientes.forEach(function(r){
      var corS = r.score>700?'#059669':r.score>=601?'#D97706':r.score>=501?'#EA580C':r.score>0?'#DC2626':'#9CA3AF';
      h += '<tr onclick="verFichaLagostao(0,\'' + (r.cnpj||'') + '\')" style="border-bottom:1px solid #EEF2F1;cursor:pointer;transition:.12s" onmouseover="this.style.background=\'#F5F7F6\'" onmouseout="this.style.background=\'\'"><td style="padding:9px 14px;font-weight:600;color:#012A2A">' + (r.cliente || '—') + '</td><td style="padding:9px 14px;color:#5F7573;font-family:monospace;font-size:11.5px">' + (r.cnpj || '—') + '</td><td style="padding:9px 14px;text-align:right"><span style="background:' + corS + '22;color:' + corS + ';padding:2px 8px;border-radius:10px;font-weight:700">' + (r.score || '—') + '</span></td><td style="padding:9px 14px;text-align:right;font-weight:700;color:#012A2A">' + R$dr(r.limite) + '</td><td style="padding:9px 14px;text-align:right;color:#5F7573">' + (r.prazo || '—') + '</td></tr>';
    });
    h += '</tbody></table></div>';
    window.abrirModalDrill('Vendedor: ' + vendedor, clientes.length + ' cliente' + (clientes.length===1?'':'s') + ' - clique para abrir ficha', h);
  };
  ...
  window.pgBILagostao = function(){
    ...
    // v4.38: chips de filtros ativos + CTA "ver os X clientes"
    const chipsAtivos = [];
    if (F.vendedor) chipsAtivos.push({label:'Vendedor', valor:F.vendedor, campo:'vendedor'});
    if (F.saida)    chipsAtivos.push({label:'Rota',     valor:F.saida,    campo:'saida'});
    if (F.risco)    chipsAtivos.push({label:'Risco',    valor:F.risco,    campo:'risco'});
    if (F.prazo)    chipsAtivos.push({label:'Prazo',    valor:F.prazo,    campo:'prazo'});
    if (F.busca)    chipsAtivos.push({label:'Busca',    valor:F.busca,    campo:'busca'});

    let breadcrumb = '';
    if (chipsAtivos.length){
      // v4.39b: "Filtro ativo" e "label:" maiores
      breadcrumb = '<div style="background:#012A2A;color:#fff;border-radius:10px;padding:12px 16px;margin-bottom:12px;display:flex;align-items:center;gap:12px;flex-wrap:wrap"><span style="font-family:Poppins,sans-serif;font-size:15px;color:#B8DDD9;text-transform:uppercase;font-weight:800;letter-spacing:.08em">Filtro ativo:</span>';
      ...
      breadcrumb += '<button onclick="var el=document.getElementById(\'bi-clientes-lista\'); if(el){el.scrollIntoView({behavior:\'smooth\',block:\'start\'});}" style="background:#15746E;color:#fff;border:none;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:700;cursor:pointer">Ver lista &darr;</button>';
      breadcrumb += '<button onclick="window._BI_LAG_FILTERS={vendedor:\'\',saida:\'\',risco:\'\',prazo:\'\',busca:\'\'};render()" style="background:transparent;color:#fff;border:1px solid rgba(255,255,255,.3);border-radius:8px;padding:6px 12px;font-size:12px;font-weight:600;cursor:pointer">Limpar tudo</button>';
      breadcrumb += '</div>';
    }
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">${cardChart('Score x Limite (radar — passe o mouse)', 'ch-scatter', 280)}${cardChart('Top 15 devedores (cor = risco de score)', 'ch-scatter-div', 380)}</div>
...
<div style="background:#fff;border:1px solid #E4EAF4;border-radius:12px;padding:0;overflow:hidden"><div style="padding:14px 18px;border-bottom:1px solid #E4EAF4;display:flex;justify-content:space-between;align-items:center"><div style="font-size:12px;font-weight:700;color:#0A3332;text-transform:uppercase;letter-spacing:.05em">Clientes filtrados (${filt.length}) — clique pra ver ficha</div><div style="font-size:11px;color:#5F7573">Ordenacao: score decrescente</div></div><div style="max-height:520px;overflow-y:auto"><table style="width:100%;border-collapse:collapse;font-size:12.5px"><thead style="background:#F5F7F6;position:sticky;top:0"><tr style="text-align:left"><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Cliente</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">CNPJ</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Vendedor</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Fundado</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Score</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Dívidas</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Prazo</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Limite</th><th style="padding:10px 14px;font-weight:700;color:#5F7573;font-size:11px;text-transform:uppercase">Saída</th></tr></thead><tbody>${filt.slice().sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,300).map((r,i)=>{const cor = r.score>700?'#059669':r.score>=601?'#D97706':r.score>=501?'#EA580C':r.score>0?'#DC2626':'#9CA3AF';return `<tr onclick="verFichaLagostao(${i},'${(r.cnpj||'').replace(/'/g,"\\'")}')" style="border-bottom:1px solid #EEF2F1;cursor:pointer" onmouseover="this.style.background='#F5F7F6'" onmouseout="this.style.background=''"><td style="padding:9px 14px;font-weight:600;color:#0A3332">${r.cliente || '—'}</td><td style="padding:9px 14px;color:#5F7573;font-family:monospace;font-size:11.5px">${r.cnpj || '—'}</td><td style="padding:9px 14px;color:#0A3332">${r.vendedor || '—'}</td><td style="padding:9px 14px;color:#5F7573">${r.fundado || '—'}</td><td style="padding:9px 14px"><span style="background:${cor}22;color:${cor};padding:2px 8px;border-radius:10px;font-weight:700;font-size:11.5px">${r.score || '—'}</span></td><td style="padding:9px 14px;color:${r.dividas>100?'#DC2626':'#5F7573'};font-weight:${r.dividas>100?'700':'500'}">${R$dr(r.dividas)}</td><td style="padding:9px 14px;color:#0A3332">${r.prazo || '—'}</td><td style="padding:9px 14px;font-weight:600;color:#0A3332">${R$dr(r.limite)}</td><td style="padding:9px 14px;color:#5F7573;font-size:11.5px">${(r.saidas||[]).join(' + ') || '—'}</td></tr>`;}).join('')}</tbody></table>${filt.length > 300 ? `<div style="padding:12px;text-align:center;color:#5F7573;font-size:12px">Mostrando primeiros 300 de ${filt.length}.</div>` : ''}</div></div>`;
  ...
  window.renderBIGraficos = function(){
    ...
    // v4.41b: Top devedores em bar chart horizontal (muito mais legivel que scatter no zero)
    const devedores = filt.filter(r=>r.dividas>0).sort((a,b)=>b.dividas-a.dividas).slice(0,15);
    const semDivida = filt.filter(r=>!r.dividas || r.dividas===0).length;
    const e9 = document.getElementById('ch-scatter-div');
    if (e9) new Chart(e9, { type:'bar', data:{ labels:devedores.map(r=>r.cliente.length>22?r.cliente.slice(0,20)+'...':r.cliente), datasets:[{ label:'Divida', data:devedores.map(r=>r.dividas), backgroundColor:devedores.map(r=>r.score>700?CORES.verdeOk:r.score>=601?CORES.amarelo:r.score>=501?CORES.laranja:r.score>0?CORES.vermelho:CORES.cinza), borderRadius:4 }]}, options:{ indexAxis:'y', responsive:true, maintainAspectRatio:false, onClick:(ev,el)=>{ if(el[0]){ var r=devedores[el[0].index]; if(r) verFichaLagostao(0, r.cnpj); }}, plugins:{ legend:{display:false}, tooltip:{callbacks:{label:c=>devedores[c.dataIndex].cliente+' | Score '+(devedores[c.dataIndex].score||'-')+' | Divida R$ '+c.parsed.x.toLocaleString('pt-BR'), afterLabel:c=>semDivida+' outros clientes com divida R$ 0 (nao mostrados)'}}}, scales:{ x:{ title:{display:true,text:'Divida (R$) — cor = risco',font:{size:11,weight:'bold'}}, grid:{color:'rgba(0,0,0,.04)'}, ticks:{font:{size:10}, callback:v=>v>=1000?'R$ '+(v/1000).toFixed(0)+'k':'R$ '+v}}, y:{ grid:{display:false}, ticks:{font:{size:10.5}}}}}});
